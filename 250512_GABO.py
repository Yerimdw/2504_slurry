...
def run_GA_BO():
    ...
    st.dataframe(pd.DataFrame(init_candidates_denorm, columns=X.columns))
    Y_BO = np.hstack([
        Y[["yield stress", "viscosity"]].values,
        X[["Graphite"]].values
    ])
    model_BO = SingleTaskGP(train_x, torch.tensor(Y_BO, dtype=torch.double))
   ...
   # model_BO = SingleTaskGP(train_x, torch.tensor(np.hstack([train_y, X[["Graphite"]].values]), dtype=torch.double))
    ...
    # train_y (yield stress)와 viscosity, graphite를 모두 포함
    train_y_hv = torch.tensor(np.hstack([
        Y.values,  # yield stress, viscosity (2개)
        X[["Graphite"]].values  # graphite (1개)
    ]), dtype=torch.double)
    ...
    candidate_scaled, _ = optimize_acqf(acq_func, bounds=normalized_bounds, q=1,
    ...
    X_tensor = train_x
    Y_tensor = torch.tensor(np.hstack([
        Y[["yield stress", "viscosity"]].values,
        X[["Graphite"]].values
    ]), dtype=torch.double)
